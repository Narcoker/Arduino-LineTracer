...
#define MOTOR_SPEED 70  //모터의 기준속력입니다(0~255), 라인트레이서는 저속주행을 권장드립니다.
#define Threshold 40
...
{
  int line_l = analogRead(A0);//digitalRead(LINESENS_L);
  int line_r = analogRead(A1);//digitalRead(LINESENS_R); //왼쪽과 오른쪽 라인센서의 감지값을 변수에 저장합니다.

  Serial.print(line_l);
  Serial.print(" ");
  Serial.println(line_r);
  
  if(line_l <= Threshold && line_r <= Threshold)      //라인이 감지되지 않을때 전진
  {
    m_a_dir = 0;  //모터A 정방향
    m_b_dir = 0;  //모터B 정방향
  ...
  else if(line_l > Threshold && line_r <= Threshold) //왼쪽 센서 감지시 왼쪽으로 전진
  {
    m_a_dir = 1;  //모터A 역방향
    m_b_dir = 0;  //모터B 정방향
    m_a_spd = MOTOR_SPEED;  //모터A의 정지
    m_b_spd = MOTOR_SPEED;  //모터B의 속력값 조정
  }
  else if(line_l <= Threshold && line_r > Threshold)  //오른쪽 센서 감지시 오른쪽으로 전진
  {
    m_a_dir = 0;  //모터A 정방향
    m_b_dir = 1;  //모터B 역방향
    m_a_spd = MOTOR_SPEED;  //모터A의 속력값 조정
    m_b_spd = MOTOR_SPEED;  //모터B의 정지
  }
  else
  {
    m_a_dir = 0;  //모터A 정방향
